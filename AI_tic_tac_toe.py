...
def wins(data) :
    ...
    #if computer wins it return 1 or if human wins it return -1 else it return 0 for draw
...
def isempty(my_board) :
    #check whether board is empty or not
    if " " in my_board:
        return True
    return False
...
def min_max(board,max_play) :
    score = wins(board)
    #three base condition to stop recursion
...
def bestmove(board) : # this function gives best move for AI
    best=-1000
    move=-1
...
def computer() :  
    global start,end
    start=time.time() #For calculating time taken by computer
